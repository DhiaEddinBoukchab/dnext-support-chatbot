...
class LLMHandler:
    ...
    # CLASSIFICATION
    # =========================
    async def classify_conversation(self, query: str) -> str:
        """
...
"""
    ...
    # =========================
    async def generate_response_stream(self, context: str, query: str, conversation_history: Optional[List[Dict]] = None) -> Generator[str, None, None]:
        """Generate streaming response using LLM with website-aware context and conversation memory"""

        conversation_type = await self.classify_conversation(query)

        if conversation_type == "CASUAL":
            prompt = self._create_casual_prompt(query)
        else:  # ACTIONABLE
            # Fetch dnext.io content
            website_context = await self.fetch_website_content("https://www.dnext.io/")
